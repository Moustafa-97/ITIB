    ...
    <>
      ...
      <PricingContainer>
        <Section>
          ...
          <Paragraph>{t("pricing.introduction.paragraph1")}</Paragraph>

          <Paragraph>{t("pricing.introduction.paragraph2")}</Paragraph>
        </Section>

        <Section>
          <Title>{t("pricing.quoteProcess.title")}</Title>
          <List>
            {getTranslationArray("pricing.quoteProcess.details").map(
              (detail, index) => (
        ...
        <Section>
          ...
          <List>
            {getTranslationArray("pricing.pricingPrinciples.details").map(
              (detail, index) => (
                <li key={index}>{detail}</li>
              )
            )}
        ...
        <Section>
          ...
          <List>
            {getTranslationArray("pricing.additionalServices.details").map(
              (detail, index) => (
                <li key={index}>{detail}</li>
              )
            )}
        ...
        <Section>
          ...
          <List>
            {getTranslationArray("pricing.generalNotes.details").map(
              (detail, index) => (
                <li key={index}>{detail}</li>
              )
            )}
